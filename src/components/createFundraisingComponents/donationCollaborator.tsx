...
export default function DonationCollaborator({ index, update, btn, singleData }: { index: number, btn?: boolean, update?: boolean, singleData?: IDonationList }) {
    ...
    return (
        <Flex>
            ...
            <ModalLayout open={open} close={() => setOpen(false)} trigger={true} >
                <Flex flexDir={"column"} w={"full"} >
                    <Flex w={"full"} px={"6"} pt={"8"} justifyContent={"space-between"} bg={secondaryBackgroundColor} >
                        <Box>
                    ...
                    <Flex px={"6"} py={"4"} flexDir={"column"} gap={"2"} bg={secondaryBackgroundColor}  >
                        {update && (
                            <Flex rounded={"lg"} justifyContent={"center"} w={"full"} bg={"#EFF1FE"} py={"3px"} px={"9px"} >
                    ...
                    {update && (
                        <Box paddingX={'6'} position={"sticky"} bottom={"0px"} shadow='lg' bg={mainBackgroundColor} py={'20px'} >
                            <CustomButton text='Update' isLoading={editDonation?.isPending} disabled={editDonation?.isPending} onClick={() => editDonation?.mutate({ collaborators: data[0].collaborators })} width='100%' height='50px' bg='brand.chasescrollButtonBlue' color={'white'} />
                        </Box>
                    )}
                </Flex>
            </ModalLayout>
