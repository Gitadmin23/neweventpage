...
export default function SelectTicketBtn(
    ...
    return (
        <Flex w={"full"} gap={"2"} flexDir={"column"} >
            ...
            <ModalLayout size="cover" closeBtn={true} open={showModal} close={() => setShowModal(false)} trigger={true} >
                <Flex w={"full"} justifyContent={"center"} alignItems={"center"} >
                    <Flex p={"5"} borderWidth={"1px"} rounded={"2xl"} gap={"4"} w={"full"} maxW={"1032px"} >
                        <Flex flexDir={"column"} alignItems={"center"} gap={"5"} px={"3"} w={"full"} >
                            ...
                            {productTypeData?.map((item, index) => {
                                return (
                                    <Flex key={index} w={"full"} borderWidth={"1px"} justifyContent={"space-between"} rounded={"8px"} px={"4"} py={"6"} >
                                        <Flex flexDir={"column"} gap={"2"} >
                                            <Text fontWeight={"semibold"} >{item.ticketType} {formatNumberWithK(item?.ticketPrice, false)}</Text>
                                            <Flex bgColor={"#FF0000"} h={"21px"} color={"white"} rounded={"md"} px={"4"} >
                                                Sales snd on {dateFormat(item.endDate)}
                                            </Flex>
                                        </Flex>
                            ...
                            <Text fontWeight={"medium"} >Powered by <span style={{ color: primaryColor }} >Chasescroll.com</span></Text>
                            <Flex w={"full"} justifyContent={"end"} mt={"auto"} >
                                <CustomButton loading={payForTicket.isPending} onClick={submitHandler} disable={selectTicketType.length > 0 ? false : true} width={"fit-content"} text={"Get Ticket"} px={"5"} borderRadius={"999px"} />
                            </Flex>
                        </Flex>
                        <Flex bgColor={secondaryBackgroundColor} rounded={"8px"} w={"fit-content"} flexDir={"column"} gap={"4"} >
                            <Flex w={"415px"} maxW={"415px"} >
                                {picUrls?.length > 0 && (
                                    <Image src={IMAGE_URL + picUrls[0]} rounded={"8px"} w={"full"} h={"328px"} />
                                )}
                            </Flex>
                            ...
                            {selectTicketType.length > 0 && (
                                <>
                                    {selectTicketType?.map((item, index) => {
                                        return (
                                            <Flex px={"3"} pb={"4"} key={index} textAlign={"left"} flexDir={"column"} gap={"4"} >
                                                <Text fontWeight={"500"} fontSize={"18px"} >Order summary</Text>
                                                <Flex w={"full"} fontWeight={"500"} justifyContent={"space-between"} alignItems={"center"} >
