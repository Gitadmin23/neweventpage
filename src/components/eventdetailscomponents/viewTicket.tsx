...
export default function ViewTicket(
    ...
    const { mutate: fetchData, isPending: isLoading } = useMutation({
        mutationKey: ['event_ticket', data?.id, user_index],
        mutationFn: () =>
            httpService.get(`/events/get-users-tickets?userID=${user_index ? user_index : user_index}&eventID=${data?.id}`),
        onSuccess: (data: any) => {
        ...
        <Flex>
            {data?.isBought && (
                <Flex w={"full"} display={[data?.isBought ? "block" : "block", "block", "block", "block"]} >
                    <CustomButton backgroundColor={"#F2F4FF"} borderWidth={"1px"} borderColor={primaryColor} color={primaryColor} borderRadius={"32px"} my={"auto"} onClick={() => setOpen(true)} disable={((!data?.isBought)) ? true : (data?.isBought) ? false : true} text={`View Ticket${length > 1 ? `s x${length}` : ""}`} width={["full"]} height={["45px"]} fontSize={"sm"} fontWeight={"semibold"} />
                </Flex>
            )}
