...
export default function DescriptionPage({ description, limit, label, width } : { description: string, label: string, limit: number, width?: string }) {
    ...
    return ( 
        <Flex w={width ?? "full"} flexDir={"column"} bgColor={secondaryBackgroundColor} p={"4"} rounded={"16px"} gap={"2"} >
            ...
            <Text fontSize={"12px"} >{textLimit(capitalizeFLetter(description), textSize)}{description?.length > limit && (<span role='button' style={{ fontWeight: "700" }} onClick={() => setTextSize((prev) => prev === description?.length ? 100 : description?.length)} >{description?.length !== textSize ? "more" : "...less"}</span>)}</Text>
        </Flex>
    )
}
